...
- Parse 4nec2 / NEC2 deck files (GW, GM, GR, GE, GN, EX, FR, RP, EN cards)
	...
	- **TL / NT**: transmission lines (NEC-2 `TL` layout, plus fnec's velocity-factor and loss-dB extensions) and two-port networks are solved as networks across the port segments' gaps, in parallel, as in NEC-2 (`--solver hallen`, voltage sources only). Validated against nec2c: `dipole-nt-tl-equiv-freesp-51seg` gives 63.35 − j213.68 Ω (nec2c 64.51 − j212.78 Ω); a 50 Ω, 0.1 m `TL` between two λ/2 dipoles gives 84.05 + j28.65 Ω (nec2c 84.826 + j31.131 Ω). An unusable card, or fnec's retired `TL … NSEG TYPE …` layout, is refused with an error (the latter with the NEC-2 rewrite)
- Hallén MoM solver — physically accurate feedpoint impedance for thin-wire antennas
  - Validated: 51-segment λ/2 dipole at 14.2 MHz → **78.83 + j42.44 Ω** (nec2c: 79.35 + j46.22 Ω; matches the Python reference `studies/mom-kernel-accuracy/hallen_reference.py`)
	...
	- Wires standing on perfect ground (GN 1) — the ground-mounted monopole — are solved by explicit images (FND-082; λ/4 monopole 39.30 + j21.38 Ω vs nec2c 39.58 + j23.21). Wires touching finite ground, or lying in or below the ground plane, fail fast with an actionable error
