...
fn hertzian_dipole_equator_vs_pole() {
    ...
    let pole = compute_hallen_fr_point_cpu(&kernel, 0.0, 0.0);
...
fn batch_computation_3_points() {
    ...
    let batch = compute_hallen_fr_batch_cpu(&kernel, &points);
...
fn azimuth_variation() {
    ...
    let phi_0 = compute_hallen_fr_point_cpu(&kernel, 90.0, 0.0);
    let phi_90 = compute_hallen_fr_point_cpu(&kernel, 90.0, 90.0);
    let phi_180 = compute_hallen_fr_point_cpu(&kernel, 90.0, 180.0);
...
fn numerical_stability_edge_cases() {
    ...
    // Very small angles
    let result_small = compute_hallen_fr_point_cpu(&kernel, 0.1, 0.1);
    assert!(result_small.gain_total_dbi.is_finite());

    // Large angles
    let result_large = compute_hallen_fr_point_cpu(&kernel, 179.9, 359.9);
...
fn polarization_components() {
    ...
    let result = compute_hallen_fr_point_cpu(&kernel, 45.0, 45.0);
