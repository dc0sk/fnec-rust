...
fn corpus_validation_dipole_freesp() {
    ...
    // Test file is inside apps/nec-cli; walk up to workspace root then into corpus/.
    ...
    // Use the test-built fnec binary path provided by Cargo.
    let output = Command::new(env!("CARGO_BIN_EXE_fnec"))
        .arg("--solver")
        .arg("hallen")
        .arg(&deck_path)
        .output()
        .expect("Failed to run fnec test binary");

    if !output.status.success() {
        panic!("fnec failed: {}", String::from_utf8_lossy(&output.stderr));
    }

    let stdout = String::from_utf8_lossy(&output.stdout);

    // Parse impedance from output: expect line like "1      26      1.000000+0.000000j   0.013471-0.002503j   74.242874+13.899516j"
    let impedance_line = stdout.lines().next().expect("fnec produced no output");

    let parts: Vec<&str> = impedance_line.split_whitespace().collect();
    assert!(
        parts.len() >= 5,
        "Unexpected fnec output format: {}",
        impedance_line
    );
