...
fn tmp(name: &str) -> PathBuf {
    ...
    p.push(format!("fnec-laplace-{name}-{n}"));
    p
}
...
fn run(extra: &[&str], deck: &PathBuf) -> std::process::Output {
...
fn laplace_rl_load_matches_equivalent_ld2_load() {
    ...
    let deck_b = tmp("b.nec");
    fs::write(&deck_b, DIPOLE.replace("{LD}", "")).unwrap();
    let cfg = tmp("loads.toml");
    fs::write(
        &cfg,
    ...
    let deck_bare = tmp("bare.nec");
    fs::write(&deck_bare, DIPOLE.replace("{LD}", "")).unwrap();
...
fn laplace_load_rejected_on_mpie_path() {
    let deck = tmp("mpie.nec");
    fs::write(&deck, DIPOLE.replace("{LD}", "")).unwrap();
    let cfg = tmp("mpie-loads.toml");
    fs::write(
        &cfg,
        "[[laplace_load]]\ntag = 1\nseg_first = 26\nnumerator = [50.0]\ndenominator = [1.0]\n",
    )
