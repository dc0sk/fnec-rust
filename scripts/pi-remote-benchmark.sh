...
Arguments:
  ssh-target          Required remote SSH target, for example: user@192.168.1.10
  remote-repo-subdir  Optional remote path under HOME (default: git/fnec-rust)
...
ssh_target="$1"
remote_repo_subdir="${2:-git/fnec-rust}"
...
ssh -o BatchMode=yes "${ssh_target}" \
  "REMOTE_REPO_SUBDIR='${remote_repo_subdir}' BENCH_DECKS='${bench_decks}' BENCH_SOLVERS='${bench_solvers}' BENCH_RUNS='${bench_runs}' bash -s" <<'EOF' > "${raw_tsv}"
set -euo pipefail
...
for deck in ${BENCH_DECKS}; do
  for solver in ${BENCH_SOLVERS}; do
    ...
    while [[ "${run}" -le "${BENCH_RUNS}" ]]; do
      ...
      if ! target/debug/fnec --solver "${solver}" "${deck}" >/tmp/fnec_out.txt 2>/tmp/fnec_err.txt; then
        status="fail"
      fi
      ...
      printf '%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n' \
        ...
        "${mode}" "${pulse_rhs}" "${freq_mhz}" "${abs_res}" "${rel_res}"
...
{
  echo "timestamp_utc,target,deck,solver,run,status,elapsed_ms,diag_mode,pulse_rhs,freq_mhz,abs_res,rel_res"
  while IFS=$'\t' read -r ts deck solver run status elapsed mode pulse_rhs freq_mhz abs_res rel_res; do
...
awk -F, '
  ...
  END {
    for (k in count) {
      split(k, parts, "|")
      avg = total[k] / count[k]
      printf "  deck=%s solver=%s runs=%d avg_ms=%.1f min_ms=%d max_ms=%d\n", parts[1], parts[2], count[k], avg, min[k], max[k]
    }
  }
' "${out_csv}"
