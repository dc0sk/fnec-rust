...
| PH9-CHK-006 | CP-002, PRT-001 | `ph9-chk-006-sommerfeld-ground.md` | `nec_solver/matrix.rs` (`image_segment` ground-image current direction) | `nec_solver/tests/ground_impedance.rs` (ground ΔZ sign/magnitude vs nec2c, two opposite-sign geometries); refreshed `nec-cli/tests/corpus_validation.rs` + `ground_diagnostics.rs` ground regressions | **Partial** 2026-07-08: fixed the sign-inverted Z-matrix ground image `(Jx,Jy,−Jz)`→`(−Jx,−Jy,+Jz)` (matches the correct far-field image); every near-ground impedance had the wrong-signed ground effect (horizontal dipole 0.1λ 92−j48 vs nec2c 52+j63). Validated by ΔZ vs nec2c: ΔR sign correct across 4 geometries, near-ground vertical +18.0 vs +18.0, PEC external-R 7→0.93 Ω; 608 tests. Finite ground still uses a normal-incidence scalar Γ; angle-dependent Fresnel (RCM) + Sommerfeld/Norton surface wave deferred. | 🔨 |
