...
| PH9-CHK-002 | CP-003, PRT-002/008 | `ph9-chk-002-junction-feed-diagnosis.md`, `ph9-chk-002-general-junction.md` | `nec_solver/geometry.rs` (`merge_collinear_wire_endpoints`, `build_conductor_paths`/`ConductorPath`), `excitation.rs` (`build_hallen_rhs_paths`), `linear.rs` (`solve_hallen_paths`, `solve_hallen_planewave_paths`), `planewave.rs` (`build_planewave_hallen_paths`), `nec-cli/solve_session.rs` (path routing + `residual_hallen_paths`) | `nec_solver/tests/collinear_merge.rs`, `nec_solver/tests/general_junction.rs` (split→single-wire exact; inverted-V R vs nec2c; path-decomposition units), `nec_solver/tests/planewave_junction.rs` (receive: split→per-wire exact; inverted-V reciprocity), `nec-cli/tests/receive_junction.rs` (CLI junctioned receive sweep + reciprocity), `nec-cli/tests/junction_feedpoint.rs` (degree-2 solves; degree-3 guarded) | **Partial** 2026-07-05: root cause = Hallén homogeneous basis resets per GW wire. Collinear case then **all degree-2 junctions** fixed via continuous conductor paths (signed arc-length `cos(k·s)` + per-path C): start-to-start split 74.41+j14.52 (was −34−j1447, == collinear), inverted-V 30°/45°/90° match nec2c radiation resistance to 2–4%; no-op elsewhere. **Receive-side plane-wave junctions** 2026-07-05: 2-DOF (cos/sin) path solver (`solve_hallen_planewave_paths` / `build_planewave_hallen_paths`) CLI-wired via `solve_plane_wave_hallen` — split-dipole receive == per-wire solver to ~1e-11, bent inverted-V reciprocity 1.5%, CLI split-dipole receive sweep matches transmit to 0.025 dB; 598 tests. Degree-3+ (T/Y), closed loops, and current-source receive-side junctions still guarded. | 🔨 |
