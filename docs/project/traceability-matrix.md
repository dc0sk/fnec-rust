...
| ID | Req | Design | Impl | Tests | Result | S |
|:---|:----|:-------|:-----|:------|:-------|:-:|
| PH8-CHK-001 | CP-003, PRT-002 | `ph8-chk-001-current-source.md` | `nec_solver/linear.rs` (`solve_hallen_current_source`), `excitation.rs` (`build_current_source_shape`) | `nec_solver/tests/current_source.rs` (Z-consistency, linearity, off-center) | **Solve core** 2026-07-02: current-source (EX type 4) Hallén solve; impedance-consistency vs voltage source 2×10⁻⁴, forced current exact; 547 tests. CLI wiring + corpus flip pending. | 🔨 |
