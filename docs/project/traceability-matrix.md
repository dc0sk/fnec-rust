...
| PH9-CHK-002 | CP-003, PRT-002/008 | `ph9-chk-002-junction-feed-diagnosis.md` | `nec_solver/geometry.rs` (`merge_collinear_wire_endpoints`), `excitation.rs` (component cos_vec/rhs), `nec-cli/solve_session.rs` (merged solve) | `nec_solver/tests/collinear_merge.rs` (chain→single-wire; no-op for single/bent/stepped) | **Partial** 2026-07-05: root cause = Hallén homogeneous basis resets per GW wire. Collinear split-conductor case fixed (chain dipole 74.41+j14.52, was −34−j1447; no-op elsewhere; 576 tests). Bends/T-junctions still guarded. | 🔨 |
