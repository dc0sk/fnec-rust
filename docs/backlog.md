...
- [x] **Sinusoidal-basis EFIE (NEC2-style Pocklington fix)**: The current pulse/continuity solver modes use a pulse-basis Pocklington EFIE that is known to diverge from the physical solution for thin-wire antennas as the segment count increases. NEC2 uses sinusoidal (piecewise-sinusoidal) basis functions via `tbf`/`sbf`/`trio` which eliminate this divergence. Implementing the same sinusoidal-basis matrix assembly makes an accurate sinusoidal path available while pulse/continuity remain experimental. Reference: xnec2c `calculations.c`, NEC2 Theory of Operation (Burke & Poggio 1981).
  - Pulse RHS normalization candidate: $$\frac{1}{dl\,\lambda}$$
	- 2026-05-30 resolution: implementation completed and tracked in roadmap `PH6-CHK-003` (status: Done, 2026-05-06). `nec_solver` now includes sinusoidal-basis assembly and CLI regression contracts confirm sinusoidal reference-dipole solves without fallback while preserving guarded residual fallback (`sinusoidal->hallen(residual)`) when needed.
	...
	- 2026-04-29 progress: extended PH2-CHK-005 current/phase corpus truth to `dipole-ground-51seg`, adding grounded current samples plus tolerance gates and tightening the checklist test so both free-space and PEC-ground dipole cases remain present in CI.
	- 2026-04-29 progress: started PH2-CHK-002 buried/near-ground guardrails by adding a fail-fast runtime diagnostic for buried-wire requests on active ground models, plus integration regression `buried_wire_with_active_ground_fails_fast_with_actionable_error` and corpus guardrail case `dipole-gn2-buried-unsupported` with expected error and forbidden-warning contracts.
	- 2026-04-29 progress: closed the supported-path half of PH2-CHK-002 by adding low above-ground GN2 corpus fixture `dipole-gn2-near-ground-51seg`, regression-gating it in ground diagnostics and corpus validation, and documenting the split between supported near-ground vs deferred buried active-ground classes.
	- 2026-04-29 progress: started PH2-CHK-004 report/table parity by adding stable `SOURCES` and `LOADS` report sections (deterministic ordering between `FEEDPOINTS` and `CURRENTS`) with new report-contract coverage for section presence/order and load-table rendering on LD corpus fixtures.
	- 2026-04-29 progress: extended PH2-CHK-004 report-contract coverage to multi-frequency load runs, locking that each per-frequency block keeps `FEEDPOINTS -> SOURCES -> LOADS -> CURRENTS` ordering and that `SWEEP_POINTS` remains a final summary section after all report blocks.
...
- [x] **Non-collinear multi-wire Hallen reformulation / Owner: Solver / Target: Phase 2**
...
- [x] **PAR-003 / Mainstream NEC workflow card coverage / Owner: Parser+Solver / Target: Phase 2 / Issue: #16**
	...
	- 2026-04-29 progress: started PH2-CHK-007 matrix traceability enforcement by adding a dedicated enforced mapping table to `docs/corpus-validation-strategy.md` (`PH2N5-001..007`) and checklist test `phase2_nec5_matrix_rows_are_traceable_to_corpus_cases` in `apps/nec-cli/tests/corpus_validation.rs`.
...
- [ ] **PAR-011 / 4nec2 solver-binary drop-in compatibility mode / Owner: CLI+Runtime / Target: Phase 4-5 / Issue: #24**
	...
	- 2026-05-30 progress: hardened CLI drop-in profile detection to the explicit NEC2MP binary-stem matrix (`nec2dxs500`, `nec2dxs1K5`, `nec2dxs3k0`, `nec2dxs5k0`, `nec2dxs8k0`, `nec2dxs11k`, case-insensitive) plus `4nec2` name detection; added regression tests that lock known-name activation and unknown `nec2dxs*` false-positive rejection.
	- 2026-05-30 progress: completed PAR-011 Phase-0 contract-capture docs slice by adding `docs/par011-dropin-contract.md` (deterministic Windows install/rollback procedure + process-contract capture template), satisfying acceptance tests `AT-PAR011-0002` and `AT-PAR011-0003` in the evidence memo.
	- 2026-05-30 progress: completed PAR-011 Phase-1 docs specification slice by freezing explicit `--exec` precedence, drop-in warning/diagnostic classes, and current scaffold side-effect constraints in `docs/par011-dropin-contract.md`, satisfying acceptance tests `AT-PAR011-0101`, `AT-PAR011-0102`, and `AT-PAR011-0103`.
	- 2026-05-30 progress: completed PAR-011 Phase-2 validation/benchmark docs slice by adding fixture schema, example record, segment-band benchmark protocol, and explicit success criteria to `docs/par011-dropin-contract.md`, satisfying acceptance tests `AT-PAR011-0201`, `AT-PAR011-0202`, and `AT-PAR011-0203`.
	- 2026-05-30 progress: bundled the remaining PAR-011 docs-only prep into the contract/evidence docs and explicitly marked the remaining work as external-evidence or implementation driven; future PAR-011 docs changes should ship together with real evidence capture rather than as separate micro-PRs.
...
- [x] **BL-IMPR-006 / GPU kernel edge-case tests**: Extend `crates/nec_accel/src/gpu_kernels` unit tests to cover 1-segment cases, very small/large frequencies, NaN-source handling, and pattern interpolation near the poles (θ=0, θ=π). Resolution criteria: dedicated edge-case tests added; existing happy-path tests untouched.
...
- [x] **BL-IMPR-010 / Pattern-interpolation accuracy property test**: Add a property-based test that asserts interpolated gain values at arbitrary (θ, φ) stay within a stated tolerance of the surrounding RP grid samples for the existing RP corpus cases. Resolution criteria: proptest added under `apps/nec-cli/tests/` (or a relevant crate) with a documented tolerance derived from current RP grid spacing.
	- 2026-05-30 resolution: added RP-grid bilinear interpolation support and property-based tolerance coverage in `crates/nec_solver/src/farfield.rs` (`proptest_pattern_interp_within_tolerance`) with documented 0.5 dBi tolerance for 10-degree grid spacing, plus public exports in `crates/nec_solver/src/lib.rs`.
- [x] **BL-IMPR-011 / Document/extend TL `NSEG` support range**: TL lossless support is implemented for NSEG=0/1/>1 with a single-section stamp; the supported `NSEG` range and any per-segment subdivision plan is not explicitly documented for users. Resolution criteria: `docs/cli-guide.md` and the staged-card status table (BL-IMPR-007) state the supported `NSEG` range; if subdivision becomes a stretch goal it is captured here as a follow-up sub-item.
	- 2026-05-30 resolution: documented supported TL `NSEG` range (0, 1, and >1) as single-section stamping with `NSEG=0` normalization in `docs/cli-guide.md` and `docs/card-support-matrix.md`.
- [x] **BL-IMPR-012 / Buried-wire detection extension**: Extend `buried_wire_geometry_error` in `apps/nec-cli/src/geometry_validation.rs` beyond the simple `z<0` test to also account for the active ground model parameters (e.g. interface depth, dielectric properties) where applicable. Resolution criteria: at least one additional regression case in `apps/nec-cli/tests/ground_diagnostics.rs` covering the extended detection path; existing buried/near-ground contracts remain green.
	- 2026-05-30 resolution: extended active-ground guardrail to reject interface-touching wires (`z <= 0`) and include finite-ground material context (`eps_r`, `sigma`) in diagnostics; added regression `interface_touching_wire_with_active_ground_fails_fast_with_material_context` in `apps/nec-cli/tests/ground_diagnostics.rs`.
- [x] **BL-IMPR-013 / User-tunable sinusoidal→Hallen residual fallback threshold**: The sinusoidal-mode residual threshold that triggers the Hallen fallback is currently hardcoded. Resolution criteria: a CLI flag (or environment variable, with CLI flag taking precedence) exposes the threshold; default behavior unchanged; diagnostic field documents the active threshold value; regression test added.
	- 2026-05-30 resolution: added `--sin-fallback-rel-max <value>` and `FNEC_SIN_FALLBACK_REL_MAX` (CLI takes precedence) with default unchanged at `1e-2`. Sinusoidal solve path now enforces fallback to Hallen when `sin_rel_res` exceeds the active threshold, emits explicit warning text, and reports `sin_fallback_rel_max` on the `diag:` line. Added regressions for CLI parse errors, invalid env handling, and CLI-over-env precedence in fallback behavior.
