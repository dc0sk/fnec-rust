...
| GN type 2 | Partial | Aliases the GN0 scalar finite-ground path by default; the true Sommerfeld surface wave is available via `--ground-solver sommerfeld` for a straight horizontal wire (feedpoint Z only, nec2c GN2, PH9-CHK-006), or via `--solver mpie` for the full near-ground **currents/patterns** on **any straight wire** (horizontal / vertical / tilted; PH9-CHK-007 Phase D+E — reproduces nec2c GN2 to <8%, e.g. a vertical λ/2 dipole 84.5+j36 vs nec2c 89.75+j38.52). Only bent/mixed geometry over ground still falls back to the scalar path |
