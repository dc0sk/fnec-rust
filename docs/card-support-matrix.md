...
| GM | Partial | Geometry move. fnec reads field I2 as a **last tag** and F7 as a **first tag**, giving a tag-range filter, and treats `I1 == 0` as move-in-place. NEC-2 defines I2 as **NRPT**, the number of new structures to generate, and F7 as **ITS**, whose meaning is the suffix of the structure from the first segment carrying that tag onward in definition order. So fnec generates at most ONE copy where a standard deck asks for NRPT, does not apply the tag increment on an in-place move, and silently does nothing where nec2c refuses an ITS naming no wire. Verified against nec2c: `GW 1 3` + `GM 10 2 0 0 0 0 0 1. 0` gives 9 segments tagged 1/11/21 in nec2c and 6 tagged 1/11 in fnec. Tracked as FND-119 |
