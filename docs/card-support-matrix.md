...
| GN type 1 | Full | Perfect-conductor (PEC) image method (correct-signed image, PH9-CHK-006). Wires standing on the ground plane (the ground-mounted monopole) are solved by explicit images on `--solver hallen` (FND-082); a wire in the plane, grounded at both ends, or several wires meeting at one ground point are refused |
